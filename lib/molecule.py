...
import cmistark.hdf5, cmistark.moleculeproperty, cmistark.util
...
class Molecule(cmistark.moleculeproperty.Molecule):
    ...
    def starkeffect(self, state, fields=None, energies=None):
        ...
        if energies == None and fields == None:
            return cmistark.hdf5.readVLArray(self.__storage, "/" + state.hdfname() + "/dcfield"), \
                cmistark.hdf5.readVLArray(self.__storage, "/" + state.hdfname() + "/dcstarkenergy"),
        elif energies == None or fields == None:
            raise SyntaxError
        else:
            assert len(fields) == len(energies)
            cmistark.hdf5.writeVLArray(self.__storage, "/" + state.hdfname(), "dcfield", fields)
            cmistark.hdf5.writeVLArray(self.__storage, "/" + state.hdfname(), "dcstarkenergy", energies)
    ...
    def starkeffect_merge(self, state, newfields=None, newenergies=None):
        ...
        try:
            ...
            fields, energies = cmistark.util.column_merge([oldfields, oldenergies], [newfields, newenergies])
